...
training with permanently more expensive inference, which training work units
alone do not show. SGD and GaussianNB nudges update parameters in place and do
not change model size at all.
...
- They are *not* bit-for-bit inert for Random Forest and SGD. They carry zero
  ...
  random stream. A check that forced them into every adaptation fit on the elec2
  stream (where none are needed) found no detectable bias in mean
...
class NudgeMechanism:
    ...
    def _nudge_partial(self, model, X, y) -> tuple[Any, CostRecord]:
        ...
        window lacks one of the stream's classes.
        """
        base = snapshot(model)
        return measure(
